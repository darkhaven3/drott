...
//   int temp;
   ...
   if (version == ROTTVERSION)
   {
      ...
      ReadBoolean("SDLScaleMethod", &sdl_scalemethod);
...
void WriteConfig (void)
{
  ...
  // Write Config, Battle and Score files
   ...
   SafeWriteString(file, "\n;\n");
   SafeWriteString(file, "; Window resolution. This can be any value \n");
   SafeWriteString(file, "; you want, as long as it's greater than \n");
   SafeWriteString(file, "; your game resolution. \n");
   WriteParameter(file,"WindowWidth      ", iGLOBAL_WINDOWWIDTH);
   WriteParameter(file,"WindowHeight     ", iGLOBAL_WINDOWHEIGHT);

   SafeWriteString(file, "\n;\n");
   SafeWriteString(file, "; SDL scaling method. \n");
   SafeWriteString(file, "; 0 - Internal (StretchMemPicture) \n");
   SafeWriteString(file, "; 1 - SDL_SoftScale \n");
   WriteParameter(file, "SDLScaleMethod      ", sdl_scalemethod);
