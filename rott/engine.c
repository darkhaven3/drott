...
void Interpolate(int x1, int x2) {
    int dx = x2 - x1;
    int dtexture = (((posts[x2].texture - posts[x1].texture) << 12) + 0x800) / dx;
    int frac = dtexture + (posts[x1].texture << 12);
    int dheight = (((posts[x2].wallheight - posts[x1].wallheight) << 8) + 0x80) / dx;
    int hfrac = dheight + (posts[x1].wallheight << 8);

    for (int i = x1 + 1; i <= x2 - 1; i++, frac += dtexture, hfrac += dheight) {
    ...
    if (vertical < 0) {
        ...
        if (posts[curx].lump & 0x4000) {
            if (tilemap[xtile - (xtilestep >> 7)][ytile] & 0x8000) {
                ...
                if (num & 0x4000) {
                    if (maskobjlist[num & 0x3ff]->sidepic) posts[curx].lump = maskobjlist[num & 0x3ff]->sidepic;
                    else posts[curx].lump &= 0x3ff;
                }
                else posts[curx].lump = doorobjlist[num & 0x3ff]->sidepic;
            }
            else {
                if (posts[curx].lump & 0x1000) posts[curx].lump = animwalls[posts[curx].lump & 0x3ff].texture;
                else posts[curx].lump &= 0x3ff;
            }
        }
        else if (posts[curx].lump & 0x2000) {
            if (IsWindow(xtile, ytile)) posts[curx].alttile = -1;
            else posts[curx].alttile = (MAPSPOT(xtile, ytile, 2)) + 1;

            posts[curx].lump &= 0x3ff;
        }
        else if (posts[curx].lump & 0x1000) posts[curx].lump = animwalls[posts[curx].lump & 0x3ff].texture;
            ...
            if ((tile = tilemap[grid[0]][grid[1]]) != 0) {
                if (tile & 0x8000) {
                    if ((!(tile & 0x4000)) && (doorobjlist[tile & 0x3ff]->action == dr_closed)) {
                        spotvis[grid[0]][grid[1]] = 1;
                        if (doorobjlist[tile & 0x3ff]->flags & DF_MULTI) MakeWideDoorVisible(tile & 0x3ff);
                        do {
                            index = (cnt >= 0);
                            cnt += incr[index];
                            grid[index] += thedir[index];
                            if ((tilemap[grid[0]][grid[1]] != 0) && (!(tilemap[grid[0]][grid[1]] & 0x8000))) break;
                        } while (1);
                        break;
                    }
                    else continue;
