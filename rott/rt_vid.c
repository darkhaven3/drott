...
void VL_MemToScreen(byte* source, int width, int height, int x, int y)
{
    ...
    for (plane = 0; plane < 4; plane++) {
        ...
        for (j = 0; j < height; j++) {
            ...
            for (i = 0; i < width; i++) *(destline + i * 4 + plane) = *ptr++;
        }
...
void VWB_TBar(int x, int y, int width, int height) {
    if (VW_MarkUpdateBlock(x, y, x + width, y + height - 1)) VL_TBar(x, y, width, height);
}
...
void VWB_Hlin(int x1, int x2, int y, int color) {
    if (VW_MarkUpdateBlock(x1, y, x2, y)) VW_Hlin(x1, x2, y, color);
}
...
void VWB_Vlin(int y1, int y2, int x, int color) {
    if (VW_MarkUpdateBlock(x, y1, x, y2)) VW_Vlin(y1, y2, x, color);
}
...
void VL_THlin(unsigned x, unsigned y, unsigned width, boolean up) {
    ...
    while (width--) {
        ...
        if (up) pixel = *(colormap + (13 << 8) + pixel);
        else pixel = *(colormap + (27 << 8) + pixel);
...
void VWB_THlin (int x1, int x2, int y, boolean up) {
   if (VW_MarkUpdateBlock (x1,y,x2,y)) VW_THlin (x1,x2,y,up);
}
...
void VWB_TVlin (int y1, int y2, int x, boolean up) {
   if (VW_MarkUpdateBlock (x,y1,x,y2)) VW_TVlin (y1,y2,x,up);
}
...
void DrawXYPic(int x, int y, int shapenum) {
    ...
    for (plane = x; plane < x + 4; plane++) {
        for (yy = 0; yy < p->height; yy++) {
            ...
            for (xx = 0; xx < p->width; xx++, buf++) *(buf + plane + xx * 4) = *(src++);
        }
    }
}
