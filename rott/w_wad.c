...
// read the entire file in
    ...
    if ((handle = open(filename, O_RDONLY | O_BINARY)) == -1) return;

    startlump = numlumps;

    if ((strcmpi(filename + strlen(filename) - 3, "wad")) && (strcmpi(filename + strlen(filename) - 3, "rts"))) {
        // single lump file
        if (!quiet) printf("    Adding single file %s.\n", filename);
        fileinfo = &singleinfo;
        ...
        if (strncmp(header.identification, "IWAD", 4)) Error("Wad file %s doesn't have IWAD id\n", filename);
        header.numlumps = IntelLong(LONG(header.numlumps));
        header.infotableofs = IntelLong(LONG(header.infotableofs));
        length = header.numlumps * sizeof(filelump_t);
        fileinfo = alloca(length);
        if (!fileinfo) Error("Wad file could not allocate header info on stack");
...
int W_CheckNumForName(char* name) {
    ...
    while (lump_p != endlump) {
        if (*(int*)lump_p->name == v1 && *(int*)&lump_p->name[4] == v2) return lump_p - lumpinfo;
        lump_p++;
    }
    return -1;
}
...
char* W_GetNameForNum(int i) {
    if (i >= numlumps) Error("W_GetNameForNum: %i >= numlumps", i);
...
void W_ReadLump (int lump, void *dest) {
        ...
        if (lump < 0) Error ("W_ReadLump: %i < 0",lump);
        l = lumpinfo+lump;

        lseek (l->handle, l->position, SEEK_SET);
        c = read (l->handle, dest, l->size);
        if (c < l->size) Error ("W_ReadLump: only read %i of %i on lump %i",c,l->size,lump);
}
...
void W_WriteLump (int lump, void *src) {
   ...
   if (lump >= numlumps) Error ("W_WriteLump: %i >= numlumps",lump);
   if (lump < 0) Error ("W_WriteLump: %i < 0",lump);
   l = lumpinfo+lump;

   lseek (l->handle, l->position, SEEK_SET);
   c = write (l->handle, src, l->size);
   if (c < l->size) Error ("W_WriteLump: only wrote %i of %i on lump %i",c,l->size,lump);
}
...
void* W_CacheLumpNum(int lump, int tag, converter_t converter, int numrec) {
    if (lump >= (int)numlumps) Error("W_CacheLumpNum: %i >= numlumps", lump);
    else if (lump < 0) Error("W_CacheLumpNum: %i < 0  Taglevel: %i", lump, tag);
