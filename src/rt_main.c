...

{
    ...
    if ((MSG.messageon == false) && (!quitactive)) {
        ...
        if (Keyboard[sc_F7] && (!BATTLEMODE || Keyboard[sc_RShift])) {
            ...
            if (!MessagesEnabled) AddMessage("Messages disabled.", MSG_MSGSYSTEM);
            else AddMessage("Messages enabled.", MSG_MSGSYSTEM);
        }
        ...
        if (Keyboard[sc_F11]) {
            ...
            gammaindex++;
            if (gammaindex == NUMGAMMALEVELS) gammaindex = 0;
            VL_SetPalette(origpal);
            itoa(gammaindex, str2, 10);
            strncat(str, str2, strlen(str2));
            AddMessage(str, MSG_SYSTEM);

            while (Keyboard[sc_F11]) IN_UpdateKeyboard();
        }
        // Increase volume
...
#if SAVE_SCREEN
        else if (Keyboard[sc_Alt] && Keyboard[sc_C]) SaveScreen(false);
#endif
    }
#ifdef USE_SDL
    ...
    if (Keystate[sc_CapsLock]) {
        ...
        if (Keystate[sc_CapsLock] == 3) Keystate[sc_CapsLock] = 0;
    }
    if (Keystate[0x45]) {   /* numlock */
        Keystate[0x45]++;
        if (Keystate[0x45] == 3) Keystate[0x45] = 0;
    }
#endif
}
